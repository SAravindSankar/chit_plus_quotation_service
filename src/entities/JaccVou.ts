...
@Index("ACC_VOUPK", ["accVoupk"], { unique: true })
@Entity("JACC_VOU")
export class JaccVou {
  @Column("number", {
    primary: true,
    name: "ACC_VOUPK",
    precision: 8,
  ...
  @Column("number", { name: "AMOUNT", nullable: true, precision: 16, scale: 2 })
  amount: number | null;
  ...
  @ManyToOne(() => MasAccounts, (masAccounts) => masAccounts.jaccVous)
  @JoinColumn([{ name: "ACCFK", referencedColumnName: "accpk" }])
  accfk: MasAccounts;

  @ManyToOne(() => MasBranch, (masBranch) => masBranch.jaccVous)
